process MMSEQS_EASYSEARCH {
    ...
    # Find MMseqs2 database files in the provided directory
    # db_target is expected to be a directory containing the database files
    if [ -d "${db_target}" ]; then
        ...
        # db_target is a file path, find database files in the same directory
        DB_DIR=\$(dirname "${db_target}")
        DB_TARGET_PATH_NAME=\$(find -L "\$DB_DIR/" -maxdepth 1 -name "${args2}" | sed 's/\\.[^.]*\$//' | sed -e 'N;s/^\\(.*\\).*\\n\\1.*\$/\\1\\n\\1/;D' )
    fi
