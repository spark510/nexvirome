#!/usr/bin/env python3
"""
Fig3-healthy diversity — Control samples only, TOOL-vs-TOOL comparison.
alpha per (cohort, tool, sample); cross-tool Kruskal-Wallis per cohort/index;
Bray-Curtis PCoA per (cohort, tool) [for the per-tool panel] AND a pooled PCoA
of all tools' Healthy samples coloured by tool.

Reads result_260605/fig3/tables/fig3_species_long.csv
Writes fig3 alpha/pcoa/kw CSVs (healthy=Control cohort).
Run: conda run -n shotgun_virome python result_260605/fig3/code/healthy_diversity.py
"""
...
# group manifest -> Healthy (Control) samples only; Fig3 is the Control cohort
GROUPS = f"{NX}/paper/figures/Fig3/source_data"


def healthy_samples():
    """Set of Control-group sample names (DNA n=5, RNA n=31)."""
    keep = set()
...
def main():
    ...
    keep = healthy_samples()
    sp = sp[sp["sample"].isin(keep)]   # Fig3 = Healthy/Control only
    ...
    # DNA vs RNA Mann-Whitney per (tool, index) — non-paired, the healthy comparison
    ...
    for tool in TOOLS:
        for idx in ("richness", "shannon", "invsimpson", "pielou"):
            ...
            mw_rows.append(dict(tool=tool, index=idx, n_dna=len(dna), n_rna=len(rna),
                                p=round(float(p), 5) if p == p else np.nan))
    pd.DataFrame(mw_rows).to_csv(f"{T}/fig3_alpha_dna_vs_rna.csv", index=False)

    # per-TOOL PCoA pooling DNA+RNA Healthy samples (one ordination per tool,
    # points coloured by cohort) + PERMANOVA(cohort, 999 perm) testing whether the
    # DNA and RNA viromes differ in Bray-Curtis structure.
        ...
        if M.shape[0] >= 3 and M.shape[1] >= 2:
            ...
            for sk, xy in zip(ss, coords):
                ...
                pooled_rows.append(dict(tool=tool, cohort=coh, sample=sname,
                                        PC1=round(float(xy[0]), 4), PC2=round(float(xy[1]), 4),
                                        var1=round(float(var[0]), 4), var2=round(float(var[1]), 4)))
            # PERMANOVA: DNA vs RNA (need >=2 per group)
            if labels.count("dna") >= 2 and labels.count("rna") >= 2:
                try:
                    ...
                    pass
    pd.DataFrame(pooled_rows).to_csv(f"{T}/fig3_pcoa_pooled.csv", index=False)
    pd.DataFrame(perm_rows).to_csv(f"{T}/fig3_permanova_dna_vs_rna.csv", index=False)
    if perm_rows:
        print("\n=== PERMANOVA DNA vs RNA (999 perm, per tool) ===")
        print(pd.DataFrame(perm_rows).to_string(index=False))

    pd.DataFrame(alpha_rows).to_csv(f"{T}/fig3_alpha.csv", index=False)
    pd.DataFrame(pcoa_rows).to_csv(f"{T}/fig3_pcoa.csv", index=False)
    pd.DataFrame(kw_rows).to_csv(f"{T}/fig3_alpha_kruskal.csv", index=False)
    pd.set_option("display.width", 140)
    print("=== Healthy alpha mean (cohort x tool) ===")
    a = pd.DataFrame(alpha_rows)
    print(a.groupby(["cohort", "tool"])[["richness", "shannon", "invsimpson"]].mean().round(2).to_string())
    print("\n=== cross-tool Kruskal-Wallis (do tools differ?) ===")
