#!/usr/bin/env python3
"""
Fig 3 (FULL cohort, DNA-vs-RNA) COMBINED — self-contained vector render.

Identical layout/styling to render_fig3_new.py (the Healthy version) but uses the
ENTIRE cohort (Control + Asthma; DNA n=24, RNA n=56, vir17 excluded upstream).
The comparison axis is still the library protocol (DNA vs RNA), not disease group.

  A  alpha diversity (Richness / Shannon / inv.Simpson), DNA vs RNA per tool
  B  Bray-Curtis PCoA per tool, DNA vs RNA, points + 95% group ellipses
  C  genus composition per tool: shared genera (>=3 tools) coloured + each tool's
     TOP-3 non-shared genera stacked on top (black border, named above)

Reads fig3/tables/*_full.csv (+ fig3_genus_long.csv) and writes
fig3/Fig3_combined_full.{eps,png}.
Run: conda run -n shotgun_virome python result_260605/fig3/code/render_fig3_full.py
"""
import os
import numpy as np
import pandas as pd
...
# ---------------------------------------------------------------- main
def main():
    alpha = pd.read_csv(f"{T}/fig3_alpha{SUF}.csv")
    mw = pd.read_csv(f"{T}/fig3_alpha_dna_vs_rna{SUF}.csv")
    pcoa = pd.read_csv(f"{T}/fig3_pcoa_pooled{SUF}.csv")
    perm = (pd.read_csv(f"{T}/fig3_permanova_dna_vs_rna{SUF}.csv")
            if os.path.exists(f"{T}/fig3_permanova_dna_vs_rna{SUF}.csv")
            else pd.DataFrame(columns=["tool", "p"]))
    gn = pd.read_csv(f"{T}/fig3_genus_long.csv"); gn = gn[gn.tool.isin(TOOLS)]
    # FULL cohort: NO Control filter — Panel C uses all samples (Control + Asthma)
    # so it matches the full-cohort alpha/PCoA panels.
    ...
    for ext in ("png", "eps"):
        fig.savefig(f"{OUT}/Fig3_combined{SUF}.{ext}", dpi=300)
    print(f"-> {OUT}/Fig3_combined{SUF}.png , .eps")
    for t in TOOLS:
