...
/*
    ...
    */
    ...
    //     ch_multiqc_files       = ch_multiqc_files.mix( CENTRIFUGE_KREPORT.out.kreport )
        ...
        ch_read1 = ch_input_for_mmseqs.map { 
            [ 
                reads: [ it.reads[0] + [id: "${it.reads[0].id}_1"], it.reads[1] ],
                db   : it.db 
            ] 
        }

        
        ch_read2 = ch_input_for_mmseqs.map { 
            [ 
                reads: [ it.reads[0] + [id: "${it.reads[0].id}_2"], it.reads[2] ],
                db   : it.db 
            ] 
        }

        ch_reads_all = ch_read1.mix(ch_read2)

        // 한 프로세스에 한번에 넘기기
        MMSEQS_TAXONOMY(ch_read1.reads,  ch_read1.db)

        ch_versions            = ch_versions.mix( MMSEQS_TAXONOMY.out.versions.first() )
        ch_raw_profiles        = ch_raw_profiles.mix(
            MMSEQS_TAXONOMY.out.db_taxonomy
...
//     }
    ...
    // mots_version   = params.run_motus ? MOTUS_PROFILE.out.versions.first() : Channel.empty()
