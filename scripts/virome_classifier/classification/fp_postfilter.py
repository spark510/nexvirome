...
module re-implements them on the COMMON output schema (the per-read `result_df`
...
result_df columns used: query, taxon_taxid, n_unique_taxids (==1 => uniquely assigned
...
def apply_fp_postfilter(
    ...
    """Filter result_df by relative-abundance (default) and/or unique-fraction
    ...
    Returns result_df with reads of rejected taxa removed. No-op when all gates off."""
    ...
    # per-species totals
    per = df.groupby("taxon_taxid").size().rename("total")
    uniq = df[is_unique].groupby("taxon_taxid").size().rename("unique")
    ...
    out = df[df["taxon_taxid"].isin(keep)].copy()
